const { garmentOptions, measurementTypes } = require("../js/catalog");
...
function getTailoringLabel(typeId) {
  return TAILORING_TYPES.find((t) => t.id === typeId)?.label || measurementTypes[typeId] || typeId || "";
}
...
function applyProductStitching(product) {
  ...
  if (!isFabric) {
    product.garmentOptions = garmentOptions(product);
    product.sizes = [...product.garmentOptions.readyMadeSizes, ...(product.garmentOptions.customEnabled ? ["Custom Stitching"] : [])];
